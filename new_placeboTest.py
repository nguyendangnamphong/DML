...
# Xác định biến điều trị (giờ là Electrical_Conductivity_IACS), biến kết quả (giờ là Ni), và biến nhiễu
treatment = 'Electrical_Conductivity_IACS'  # Biến điều trị giả (thay thế Ni)
outcome = 'Ni'  # Biến kết quả giả (thay thế Electrical_Conductivity_IACS)
...
T = df[treatment].values  # Biến điều trị giả
Y = df[outcome].values  # Biến kết quả giả
...
dml = DML(
    ...
    discrete_treatment=False,    # Biến liên tục
...
# In kết quả
print("Hiệu ứng nhân quả trung bình của Electrical_Conductivity_IACS lên Ni:", dml.const_marginal_effect(X_test).mean())
try:
...
plt.plot(effect, label='Hiệu ứng nhân quả của Electrical_Conductivity_IACS lên Ni')
plt.axhline(0, color='gray', linestyle='--')
plt.title('Placebo Test: Hiệu ứng của Electrical_Conductivity_IACS lên Ni')
plt.xlabel('Mẫu kiểm tra')
plt.ylabel('Hiệu ứng')
plt.legend()
plt.savefig('E:/Khóa luận/Data/placebo_effect_plot.png')
plt.show()